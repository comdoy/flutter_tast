...
class _EnrollPageState extends State<EnrollPage> {
  ...
  doRegister() async {
    ...
    if (!regex.hasMatch(username)) {
      ...
      var api = '${Config.domain}/user/register';
      ...
      if (response.data["code"] == '0') {
        ...
        var api = '${Config.domain}/edit_advantage/add_advantage';
        var apia = '${Config.domain}/edit_career/add_career';
        var apib = '${Config.domain}/edit_educate/add_educate';
        var apic = '${Config.domain}/edit_project/add_project';
        var responses =
            await Dio().post(api, data: {"user_id":userInfo["id"]});
        var responsea = await Dio()
            .post(apia, data: {"user_id": userInfo["id"]});
        var responseb = await Dio()
            .post(apib, data: {"user_id": userInfo["id"]});
        var responsec = await Dio()
            .post(apic, data: {"user_id": userInfo["id"]});
        if (responses.data["code"] == '0' &&
            responsea.data["code"] == "0" &&
            responseb.data["code"] == '0' &&
            responsec.data["code"] == "0") {
          await Storage.setString(
              'editresumedetails', json.encode(responses.data["data"]));
          await Storage.setString(
              "editCareer", json.encode(responsea.data["data"]));
          await Storage.setString(
              "editEducate", json.encode(responseb.data["data"]));
          await Storage.setString(
              "editProject", json.encode(responsec.data["data"]));
